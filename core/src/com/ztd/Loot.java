...
public class Loot {
    float x, y, w, h, speed, ix, iy, fx, fy, angle;       // ix = initial x, fx = final x
    Texture ltx;
    int item_value;
    String type;
    boolean active = true;
    ...
    public Loot(String type, float x, float y, float fx, float fy){
        ...
        w = ltx.getWidth();
        h = ltx.getHeight();
        item_value = UI.zombie_value.get(UI.last_zombie) == null ? 5 : UI.zombie_value.get(UI.last_zombie);
        speed = 2;
    }
    ...
    public void moveToTarget(Loot l){
        getAngle(l);
